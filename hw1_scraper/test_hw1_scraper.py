...
def test_yelp_search():
    with mugrade.test: assert yelp_search("Pittsburgh")[0] == 50 
    with mugrade.test: assert [[k in b for k in ['id', 'name', 'phone', 'review_count']] for b in yelp_search("Pittsburgh")[1]] == [[True]*4]*50
...
def test_all_restaurants():
    ...
    with mugrade.test: assert len(restaurants) == 240 
